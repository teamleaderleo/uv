...
mode=${1:?usage: relocatable-launcher-portability.sh MODE}
case "$mode" in
  gnu|busybox) ;;
  *)
...
root=$(mktemp -d "${TMPDIR:-/tmp}/uv-launcher-portability.XXXXXX")
cleanup() {
  status=$?
...
for variant in current no-delimiters; do
