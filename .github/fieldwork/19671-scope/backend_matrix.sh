...
printf 'backend\tinit_status\tbuild_status\thyphen_pyi\thyphen_py\tunderscore_py\tproject_scripts\n' >"$out/summary.tsv"
...
for backend in "${backends[@]}"; do
    ...
    if [[ "$init_status" != 0 ]]; then
        printf '%s\t%d\tNA\t0\t0\t0\t0\n' "$backend" "$init_status" >>"$out/summary.tsv"
        continue
    fi
    ...
    printf '%s\t%d\t%d\t%d\t%d\t%d\t%d\n' \
        "$backend" "$init_status" "$build_status" \
...
for backend, row in rows.items():
    if backend == "uv":
        continue
    assert row["underscore_py"] == "1", (backend, row)
    assert row["hyphen_pyi"] == "0" and row["hyphen_py"] == "0", (backend, row)
    assert row["project_scripts"] == "1", (backend, row)
    assert row["build_status"] == "0", (backend, row)
PY
