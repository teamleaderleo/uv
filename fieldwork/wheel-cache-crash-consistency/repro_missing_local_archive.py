#!/usr/bin/env python3
"""Probe whether a local-wheel cache pointer heals a missing extracted archive."""
...
PACKAGE_NAME = "fieldwork-cache-missing-archive"
MODULE_NAME = "fieldwork_cache_missing_archive"
VERSION = "0.0.1"
...
def build_wheel(destination: Path) -> Path:
    ...
    files: dict[str, bytes] = {
        ...
        f"{dist_info}/WHEEL": (
            "Wheel-Version: 1.0\n"
            "Generator: fieldwork-cache-missing-archive\n"
...
def pointer_receipts(cache: Path) -> list[dict[str, Any]]:
    ...
    for pointer in sorted(cache.rglob("*.rev")):
        if pointer.is_file():
            stat = pointer.stat()
            receipts.append(
                {
...
def main() -> int:
    ...
    temp = Path(tempfile.mkdtemp(prefix="uv-cache-missing-archive-probe-"))
    try:
        ...
        first_venv, first_install = install_once(
            uv, work, cache, wheel, "venv-first"
        )
        first_verify = (
        ...
        second_venv, second_install = install_once(
            uv, work, cache, wheel, "venv-second"
        )
        second_verify = (
        ...
        receipt = {
            "evidence_class": "target-executed-missing-archive-model",
            "outcome": outcome,
