...
root="${RUNNER_TEMP:-/tmp}/uv-root-lookahead-${UV_VERSION}"
...
if parent_child is None:
    raise SystemExit("control invalid: parent metadata has no child requirement")
...
lines = [
    f"# UV root-versus-lookahead control — {records[0]['uv_version']}",
    "",
    "Root declaration: **absolute**. Local-parent source context: **relative**.",
    "",
...
absolute_sources = sum(record["child_source_absolute"] for record in records)
relative_metadata = sum(not record["parent_metadata_absolute"] for record in records)
...
lines.extend(
    [
        ...
        f"Absolute child source records: **{absolute_sources}/{len(records)}**.",
        f"Relative parent metadata records: **{relative_metadata}/{len(records)}**.",
    ]
)
summary_path.write_text("\n".join(lines) + "\n")

if absolute_sources != len(records):
    raise SystemExit("root absolute presentation was not preserved")
if relative_metadata != len(records):
    raise SystemExit("control invalid or lookahead-relative context was lost")
if len(source_values) != 1 or len(metadata_values) != 1:
    raise SystemExit("dependency order changed the selected presentation")
PY

cat "$summary"

mkdir -p fieldwork/source-provenance-probe/out-root-lookahead
cp "$results" fieldwork/source-provenance-probe/out-root-lookahead/results.jsonl
cp "$summary" fieldwork/source-provenance-probe/out-root-lookahead/summary.md
